...
namespace TechnicalKnowledge.Repository
{
    public class LocadoraRepository
    {
        ...
        public TechnicalKnowledge.Models.Locacao Disponibilidade(string nome_filme)
        {
            try
            {
                var result = (from c in db.Filme
                              ...
                              select new TechnicalKnowledge.Models.Locacao { id_cliente = (int)t.id_cliente, id_filme = c.id_filme }
                          ).OrderByDescending(c => c.data_locacao).FirstOrDefault();
                ...
                return new Models.Locacao() { id_cliente = result.id_cliente, id_filme = result.id_filme };
            }
            catch (Exception ex)
            {
                throw ex;
